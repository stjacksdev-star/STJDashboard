...
function exportExcel() {
    ...
    ['country', 'period', 'tab', 'brand', 'category', 'search'].forEach(key => { if (filters[key] !== '' && filters[key] !== null) params.set(key, filters[key]); });
    window.location.href = `/dashboard-api/reports/product-performance/export?${params.toString()}`;
}
    ...
    <AdminLayout>
        <div class="mx-auto max-w-[1550px] space-y-6 p-4 sm:p-6">
            ...
            <section class="app-surface overflow-hidden rounded-lg border shadow-sm" style="border-color: var(--stj-border);">
                <div class="flex flex-wrap items-center justify-between gap-3 border-b p-3" style="border-color: var(--stj-border);"><div class="flex flex-wrap gap-1"><button v-for="tab in tabs" :key="tab.id" class="rounded-md px-4 py-2 text-sm font-semibold" :class="filters.tab === tab.id ? 'bg-blue-600 text-white' : 'app-surface-soft app-text'" @click="selectTab(tab.id)">{{ tab.label }}</button></div><button :disabled="loading" class="rounded-md bg-emerald-600 px-4 py-2 text-sm font-semibold text-white disabled:opacity-50" @click="exportExcel">Descargar Excel</button></div>
